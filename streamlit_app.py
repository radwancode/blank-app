...
import random
from quiz_data import quiz_data  # Make sure to import the dataset correctly
...
def run_quiz():
    ...
    for idx, question in enumerate(selected_questions):
        ...
        # Radio buttons for choices, store answers in session state
        answer = st.radio("Choose an answer:", options, key=f"q{idx}")

        if answer is not None:
            st.session_state.answers[question['country']] = (answer, correct_answer)

    # Submit button to process and show results
    if st.button("Submit Quiz"):
        score = 0
        for question in selected_questions:
            answer, correct_answer = st.session_state.answers.get(question['country'], (None, None))
        ...
        if st.button("Next Set of Questions"):
            st.session_state.answers = {}  # Clear answers for next set
